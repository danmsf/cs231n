...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores, fc_cache = affine_forward(affine_relu_out, W2, b2)
        ...
        dhidden_layer, dw2, db2 = affine_backward(dscores, fc_cache)
        dx, dw1, db1 = affine_relu_backward(dhidden_layer, affine_relu_cache)
        dw1 += self.reg*W1
        dw2 += self.reg*W2
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i, hidden_dim in enumerate(hidden_dims):
          if i==0:
            self.params['W'+str(i+1)] = np.random.randn(input_dim, hidden_dim)*weight_scale
          else:
            self.params['W'+str(i+1)] = np.random.randn(hidden_dims[i-1], hidden_dim)*weight_scale
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
          ...
          if self.use_batchnorm:
            ...
            inputs, cache = affine_batch_relu_forward(inputs, W, b,  gamma, beta, bn_param)
          else:
          ...
          cache_val['cache'+str(i+1)] = cache
        
        # final layer
        i +=1
        W, b = self.params['W'+str(i+1)], self.params['b'+str(i+1)]
        scores, cache = affine_forward(inputs, W, b)
        cache_val['cache'+str(i+1)] = cache
        ...
        def affine_batch_relu_backward(dout, cache):
          ...
          da2, dgamma, dbeta  = batchnorm_backward(da, batch_cache)
        ...
        for i in range(self.num_layers, -1, -1):
            W, b, cache = self.params['W'+str(i+1)], self.params['b'+str(i+1)], cache_val['cache'+str(i+1)]
            loss += 0.5*self.reg*np.sum(W**2)
            if i==self.num_layers:
              dprev_layer, dw, db = affine_backward(dscores, cache)
            else:
              if self.use_dropout:
                dprev_layer, cache = dropout_relu_backward(dprev_layer, cache)
              if self.use_batchnorm:
                dprev_layer, dw, db, dgamma, dbeta = affine_batch_relu_backward(dprev_layer, cache)
                grads['gamma'+str(i+1)] = dgamma
                grads['beta'+str(i+1)] = dbeta
              else:
                dprev_layer, dw, db = affine_relu_backward(dprev_layer, cache)
